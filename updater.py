...
class Matches():
    ...
    def searchIndexes(self,query):
        matches = self.df[self.df.apply(lambda row: query in ' '.join(row.values.astype(str)), axis=1)].index.tolist()
        return matches
...
class Apotheke():
    def __init__(self,file_name):
        ...
        self.df = pd.read_csv('data\\'+file_name, index_col='name')
    ...
    def updateDataframe(self):
        self.df = pd.read_csv('data\\'+self.file_name, index_col='name')
...
class App():
    ...
    def getBestDeal(self, id_list1, id_list2, anzahl1, anzahl2):
        ...
        for apo in all_columns:
            ...
            if bestdeal_dict == {} or bestdeal_dict['total'] > total:
                bestdeal_dict['total'] = total
                bestdeal_dict['apotheke'] = apo
                bestdeal_dict['Strain1'] = cheapest_id1
                bestdeal_dict['Strain2'] = cheapest_id2
        if not bestdeal_dict:
            return None
        result_string = ('Bestes Angebot bei: <b>'+ str(bestdeal_dict['apotheke']) +'</b>\n'
                 'Preis ohne Versandkosten: <b>' + str(bestdeal_dict['total']) + '</b>\n\n'
